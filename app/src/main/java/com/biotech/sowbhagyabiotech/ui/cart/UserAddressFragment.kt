...
class UserAddressFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val fullnames = securedSharedPreferenceWrapper.getString("edit_fullname", "").toString()
        val mnum = securedSharedPreferenceWrapper.getString("edit_mnum", "").toString()
        val hn = securedSharedPreferenceWrapper.getString("edit_hn", "").toString()
        val lmark = securedSharedPreferenceWrapper.getString("edit_lmark", "").toString()
        val loc = securedSharedPreferenceWrapper.getString("edit_loc", "").toString()
        val cit = securedSharedPreferenceWrapper.getString("edit_cit", "").toString()
        val stat = securedSharedPreferenceWrapper.getString("edit_stat", "").toString()
        val pin = securedSharedPreferenceWrapper.getString("edit_pin", "").toString()
        address_id = securedSharedPreferenceWrapper.getInt("edit_id", 0).toString()
        ...
        binding.updateadress.setOnClickListener {
           ...
           val email = binding.emailEdit.text.toString()
            ...
            if (fullname.isEmpty()) {
                ...
                var updatedAddressId: String = ""
                if(binding.actUpdate.text.toString().trim() == "Create"){
                    updatedAddressId = "0"
                }else{
                ...
                /*if(address_id=="-1")
                    address_id="0"*/
                homeViewModel.calluseraddressApi(requireActivity(),
                    securedSharedPreferenceWrapper.getString(ApiConstants.USER_ID, ""),
                    fullname, email_id = email, mobile_number = number,
                    pincode = pincode, address = hno, apartment = landmark,
                    street_address = locality, city = city, state = state, country = "India",address_id=updatedAddressId
    ...
    override fun onResume() {
        super.onResume()
        (activity as Home_Activity).setHeaderTitleVisible("Add Address")
    }
